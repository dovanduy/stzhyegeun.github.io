...
( function(global, factory) {
	...
	var RealJS = function() {
		...
		this.realUpdateUserTrophy = function(inRealId, inValue, inCallback, inContext) {
			if (typeof window.$ === 'undefined') {
				this.realState = EState.NONE;
				return false;
			}
			
			var reqUrl = "https://html5.stzapp.net:11001/api/user/trophy";
			var postData = {
				'user_id': inRealId, 
				'trophy': inValue
			};
			// NOTE @hyegeun GET에서 POST로 바뀌면 이 줄 제거
			reqUrl = reqUrl + '?user_id=' + postData.user_id + '&trophy=' + postData.trophy;
			
			$.post(reqUrl, postData, function(res) {
				if (!res) {
					throw new Error('[realjs-on] GET user info failed!!');
				}
		...
		// 로비 입장 - JOIN_LOBBY
		this.realJoinLobby = (function(isBlock, inContextId) {
			if (isBlock && _isServerWaiting === true) {
				return false;
			}
			var type = "ROOM:JOIN";
			var roomId = "lobby" + (inContextId ? ":" + inContextId : "");
			var data = {
				"type": type, 
				"room_id": roomId
			};
