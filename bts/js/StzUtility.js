...
StzUtil.getScreenCapture = function(inGameContext, inX, inY, inWidth, inHeight, inScale, inTargetCanvas) {
	...
	if (inGameContext.renderType === Phaser.CANVAS) {
		...
		var captureXOffset = (captureHeight * 2 - captureWidth);
		captureXOffset = (captureXOffset < 0 ? 0 : captureXOffset);
		
		var captureBMD = inGameContext.make.bitmapData(captureWidth * captureScale + captureXOffset, captureHeight * captureScale);
		...
		if(inTargetCanvas){
			...
			captureBMD.copy(inGameContext.canvas, captureX, captureY, captureWidth, captureHeight, captureXOffset / 2, 0, captureWidth, captureHeight, null, null, null, captureScale, captureScale);
		}
...
StzUtil.moveSpriteByQuadraticBezierCurve = function(game, inSprite, inFromPoint, inCentralPoint, inToPoint, operateTime, inCallback, inContext) {
