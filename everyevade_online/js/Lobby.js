...
Lobby.prototype.create = function() {
	...
	this._btnInputDone.events.onInputDown.add(function () {                
		...
		window.socket = io.connect('http://localhost:9999');
		window.socket.emit('userinit', {id: this._inputField.text.text});
		window.socket.on('startlevel', (function(data) {
			this.game.state.start('Level', true, false, data.player_id);
		}).bind(this));
	}, this);
